...
.modal{background:var(--surface);border-radius:var(--radius-lg);border:1px solid var(--border);padding:1.75rem;width:100%;max-width:540px;max-height:92vh;overflow-y:auto}
...
<div class="app">
  <div class="header">
    <div class="header-left">
      ...
      <p><span class="sync-dot ok"></span>Running locally &mdash; data saved on your computer</p>
    </div>
  ...
  <div class="toolbar">
    <div class="search-wrap">
      ...
      <input id="searchInput" placeholder="Search by name, ID, border number, employer…" oninput="renderTable()">
    </div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Employer</th>
          <th onclick="sortBy('lastVacStart')">Last vacation &#8597;</th>
...
function vacStatus(e){const ds=daysDiff(e.nextVacStart),de=daysDiff(e.nextVacEnd);if(!e.nextVacStart)return{label:'Not scheduled',cls:'badge-gray'};if(ds<=0&&de>=0)return{label:'On vacation',cls:'badge-blue'};if(ds>0&&ds<=30)return{label:`In ${ds}d`,cls:'badge-amber'};if(ds>0)return{label:`In ${ds}d`,cls:'badge-green'};return{label:'Past',cls:'badge-gray'}}
function loanStatus(e){if(!e.loanAmount||e.loanAmount<=0)return{label:'No loan',cls:'badge-gray'};if(!e.loanRemaining||e.loanRemaining<=0)return{label:'Paid off',cls:'badge-green'};const pct=Math.round((e.loanRemaining/e.loanAmount)*100);if(pct>60)return{label:`${pct}% left`,cls:'badge-red'};if(pct>20)return{label:`${pct}% left`,cls:'badge-amber'};return{label:`${pct}% left`,cls:'badge-green'}}
...
function renderTable(){
  ...
  let rows=[...employees];
  if(q)rows=rows.filter(e=>[e.name,e.empId,e.borderNo,e.employerName,e.employerId].join(' ').toLowerCase().includes(q));
  ...
  if(!rows.length){tbody.innerHTML=`<tr><td colspan="12"><div class="state-box"><p>&#128100; No employees found</p><p class="sub">Add your first employee using the button above</p></div></td></tr>`;renderStats();return}
  tbody.innerHTML=rows.map((e,i)=>{
    const vs=vacStatus(e),ls=loanStatus(e),oi=employees.indexOf(e),yr=yearsFrom(e.joiningDate);
    return`<tr><td style="color:var(--text3);font-size:11px">${i+1}</td><td class="td-name">${e.name}</td><td class="td-mono">${e.empId||'—'}</td><td class="td-mono">${e.borderNo||'—'}</td><td><span style="font-size:12px">${dFmt(e.joiningDate)}</span>${yr?`<span style="font-size:10px;color:var(--text3);margin-left:5px">${yr}y</span>`:''}</td><td><div style="font-size:12px;font-weight:500">${e.employerName||'—'}</div><div class="td-mono" style="font-size:10px;margin-top:1px">${e.employerId||''}</div></td><td style="font-size:11px">${e.lastVacStart?dFmt(e.lastVacStart)+(e.lastVacEnd?' – '+dFmt(e.lastVacEnd):''):'—'}</td><td style="font-size:11px">${e.nextVacStart?dFmt(e.nextVacStart)+(e.nextVacEnd?' – '+dFmt(e.nextVacEnd):''):'—'}</td><td><span class="badge ${vs.cls}">${vs.label}</span></td><td style="font-size:12px">${e.loanRemaining>0?fmtSAR(e.loanRemaining):'—'}</td><td><span class="badge ${ls.cls}">${ls.label}</span></td><td><div class="row-actions"><button class="btn btn-icon" onclick="openDetail(${oi})">&#128065;</button><button class="btn btn-icon" onclick="openEdit(${oi})">&#9998;</button><button class="btn btn-icon" onclick="deleteEmp(${oi})" style="color:var(--red)">&#128465;</button></div></td></tr>`;
  }).join('');
  renderStats();
}
...
function renderStats(){
  ...
  const avgT=employees.length?(employees.reduce((s,e)=>s+(e.joiningDate?parseFloat(yearsFrom(e.joiningDate)):0),0)/employees.length).toFixed(1):'—';
  document.getElementById('statsRow').innerHTML=`<div class="stat-card"><div class="lbl">Employees</div><div class="val">${employees.length}</div></div><div class="stat-card"><div class="lbl">On vacation</div><div class="val">${onVac}</div></div><div class="stat-card"><div class="lbl">Vacation in 30d</div><div class="val">${upcoming}</div></div><div class="stat-card"><div class="lbl">Active loans</div><div class="val">${withLoan}</div></div><div class="stat-card"><div class="lbl">Total loan left</div><div class="val sm">SAR ${totalLoan.toLocaleString()}</div></div><div class="stat-card"><div class="lbl">Avg. tenure</div><div class="val">${avgT}<span style="font-size:14px;font-weight:400;color:var(--text2)">y</span></div></div>`;
}
...
function clearForm(){['f_name','f_empId','f_borderNo','f_joiningDate','f_employerName','f_employerId','f_lv_start','f_lv_end','f_nv_start','f_nv_end','f_loan','f_loan_rem','f_loan_pay'].forEach(id=>document.getElementById(id).value='')}
function fillForm(e){document.getElementById('f_name').value=e.name||'';document.getElementById('f_empId').value=e.empId||'';document.getElementById('f_borderNo').value=e.borderNo||'';document.getElementById('f_joiningDate').value=e.joiningDate||'';document.getElementById('f_employerName').value=e.employerName||'';document.getElementById('f_employerId').value=e.employerId||'';document.getElementById('f_lv_start').value=e.lastVacStart||'';document.getElementById('f_lv_end').value=e.lastVacEnd||'';document.getElementById('f_nv_start').value=e.nextVacStart||'';document.getElementById('f_nv_end').value=e.nextVacEnd||'';document.getElementById('f_loan').value=e.loanAmount||'';document.getElementById('f_loan_rem').value=e.loanRemaining||'';document.getElementById('f_loan_pay').value=e.loanMonthly||''}

async function saveEmployee(){
  const emp={name:document.getElementById('f_name').value.trim(),empId:document.getElementById('f_empId').value.trim(),borderNo:document.getElementById('f_borderNo').value.trim(),joiningDate:document.getElementById('f_joiningDate').value,employerName:document.getElementById('f_employerName').value.trim(),employerId:document.getElementById('f_employerId').value.trim(),lastVacStart:document.getElementById('f_lv_start').value,lastVacEnd:document.getElementById('f_lv_end').value,nextVacStart:document.getElementById('f_nv_start').value,nextVacEnd:document.getElementById('f_nv_end').value,loanAmount:Number(document.getElementById('f_loan').value)||0,loanRemaining:Number(document.getElementById('f_loan_rem').value)||0,loanMonthly:Number(document.getElementById('f_loan_pay').value)||0};
  ...
  try{
    if(editIdx!==null){
      await fetch(`/api/employees/${editIdx}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(emp)});
...
function openDetail(i){
  const e=employees[i],vs=vacStatus(e),ls=loanStatus(e);
  ...
  document.getElementById('detailContent').innerHTML=`<div class="d-header"><div class="avatar">${initials(e.name)}</div><div><div class="d-name">${e.name}</div><div class="d-sub">${e.empId||''}${e.borderNo?' · Border: '+e.borderNo:''}</div></div></div><div class="d-grid"><div class="d-cell"><div class="d-cell-lbl">Joining date</div><div class="d-cell-val">${dFmt(e.joiningDate)}${tenure?' ('+tenure+'y)':''}</div></div><div class="d-cell"><div class="d-cell-lbl">Border number</div><div class="d-cell-val" style="font-family:var(--mono);font-size:12px">${e.borderNo||'—'}</div></div><div class="d-cell"><div class="d-cell-lbl">Employer</div><div class="d-cell-val">${e.employerName||'—'}</div></div><div class="d-cell"><div class="d-cell-lbl">Employer ID</div><div class="d-cell-val" style="font-family:var(--mono);font-size:12px">${e.employerId||'—'}</div></div></div><div class="hl hl-green"><div class="hl-label">&#127958; Vacation</div><div class="hl-value">${nv}</div><div class="hl-sub">${nd?nd+' · ':''} Last: ${lv}</div></div>${e.loanAmount>0?`<div class="hl hl-amber"><div class="hl-label">&#128181; Loan</div><div class="hl-value">${fmtSAR(e.loanRemaining||0)} remaining</div><div class="hl-sub">Total: ${fmtSAR(e.loanAmount)} · Paid: ${fmtSAR(loanPaid)}${monthsLeft?' · ~'+monthsLeft+' months left':''}</div></div>`:''}<div class="d-grid" style="margin-top:0"><div class="d-cell"><div class="d-cell-lbl">Vacation status</div><div class="d-cell-val"><span class="badge ${vs.cls}">${vs.label}</span></div></div><div class="d-cell"><div class="d-cell-lbl">Loan status</div><div class="d-cell-val"><span class="badge ${ls.cls}">${ls.label}</span></div></div>${e.loanMonthly>0?`<div class="d-cell"><div class="d-cell-lbl">Monthly payment</div><div class="d-cell-val">${fmtSAR(e.loanMonthly)}</div></div>`:'<div class="d-cell"></div>'}${monthsLeft?`<div class="d-cell"><div class="d-cell-lbl">Months to payoff</div><div class="d-cell-val">${monthsLeft} months</div></div>`:'<div class="d-cell"></div>'}</div>`;
...
if __name__ == '__main__':
    ...
    print(f'Employee Tracker running at http://localhost:{port}')
